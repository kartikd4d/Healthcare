...
use App\Models\ServicesQualifications;
use App\Models\Suitablility;
...
class ChecklistController extends Controller
{
    ...
    public function ServiceQualification(Request $req)
    {
        try {
            // $data = new ChecklistDetail;
            $result = ServicesQualifications::create($req->all());
            // $result = $data->save();
            if ($result) {
                return response()->json([
                    "message" => "checklist detail submit successfully",
...
public function Suitablility(Request $req)
    {
        try {
            // $data = new ChecklistDetail;
            $result = Suitablility::create($req->all());
            // $result = $data->save();
            if ($result) {
                return response()->json([
                    "message" => "Suitablility detail submit successfully",
                    'data' => $result,
                ], 200);
            } else {
                return response()->json([
            ...
            return response()->json([
                "message" => "create Suitablility request failed",
                "error" => $ex->getMessage()
            ], 500);
        }
    }

public function Declaration(Request $req)
    {
        try {
            // $data = new ChecklistDetail;
            $result = Declaration::create($req->all());
            // $result = $data->save();
            if ($result) {
                return response()->json([
                    "message" => "Declaration detail submit successfully",
                    'data' => $result,
                ], 200);
            } else {
                return response()->json([
            ...
            return response()->json([
                "message" => "create declaration request failed",
                "error" => $ex->getMessage()
            ], 500);
        }
    }
