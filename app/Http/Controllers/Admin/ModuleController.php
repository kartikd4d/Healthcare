...
class ModuleController extends Controller
{
    ...
    function Modulelist(Request $req)
    {
        try {
            ...
            ->select('modules.module_name', 'modules.module_status','modules.id as module_id', DB::raw('count(documents.module_id) as total_document'))
            ->groupBy("modules.id");
    ...
    function Moduleupdate(Request $req)
    {
        try {
            ...
            if ($validator->fails()) {
                ...
                if($result){
                    return response()->json([
                        "message" => "Module record update successfully",
                        "data" => $result,
                    ], 200);
                } else {
                    return response()->json([
            ...
            return response()->json([
                'message' => 'Module update request failed',
                'error' => $ex->getMessage(),
            ], 500);
        }
    ...
    function DeleteModule($key)
    {
        try {
            ...
            if ($result) {
                return response()->json([
                    "message" => "Module delete successfully $key",
                    "success"=>true,
                    'data' => $result,
                ], 200);
            } else {
            ...
            return response()->json([
                'message' => 'Module delete request failed',
                'error' => $ex->getMessage(),
            ], 500);
        }
